# THIS SCRIPT CUSTOMIZED FOR SEURAT V5 
...
#' }
#' @export
write_h5 <- function(fn, seurat, layers, assays = c("RNA", "peaks", "GeneActivity"),join_rna) {
  ...
  for (assay in assays) {
    if (!assay %in% names(seurat@assays)) next
    
    if (assay == "RNA") {
      # Seurat v5: only join if requested AND there are multiple layers
      if (join_rna && length(SeuratObject::Layers(seurat[["RNA"]])) > 1) {
        seurat <- SeuratObject::JoinLayers(seurat, assay = "RNA")
      }
      
      counts.matrix <- SeuratObject::LayerData(seurat, assay = "RNA", layer = "counts")
      write_h5_extra_assay(fn, counts.matrix, assay)
      
    } else if (assay %in% c("peaks", "GeneActivity")) {
