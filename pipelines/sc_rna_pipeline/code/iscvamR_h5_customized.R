...
write_h5 <- function(fn, seurat, layers, assays = c("RNA", "peaks", "GeneActivity")) {
  ...
  for (assay in assays) {
    if (assay %in% names(seurat@assays)) {
      ...
      counts.matrix <- GetAssayData(seurat, assay = assay, layer = "counts")
...
layer_artifacts_from_seurat <- function(seurat, qc_features, umap = "umap", tsne = "tsne",
  ...
  ####hard-code fix orig.ident
  # quick hard-code: every cell comes from "acral_2101"
  #seurat$orig.ident <- rep("acral_2101", ncol(seurat))
